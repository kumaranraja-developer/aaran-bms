@props([
    ...
    'content' => 'Click here to learn about our latest feature'
])
...
<div x-data="{
    ...
    <div class="flex items-center justify-between w-full h-full px-3 mx-auto max-w-7xl ">
        <a href="#"
           class="flex flex-col w-full h-full text-xs leading-6 text-white duration-150 ease-out sm:flex-row sm:items-center opacity-80 hover:opacity-100">
            <span class="flex items-center">
                ...
                <strong class="font-semibold">{{$title}}</strong><span
                    class="hidden w-px h-4 mx-3 rounded-full sm:block bg-neutral-700"></span>
            </span>
            <span class="block pt-1 pb-2 leading-none sm:inline sm:pt-0 sm:pb-0">{{$title}}</span>
        </a>
        <button @click="bannerVisible=false;"
{{--        setTimeout(()=>{ bannerVisible = true }, 1000);"--}}
                class="flex items-center flex-shrink-0  translate-x-1 ease-out duration-150 justify-center w-6 h-6 p-1.5 text-white rounded-full hover:bg-neutral-800">
