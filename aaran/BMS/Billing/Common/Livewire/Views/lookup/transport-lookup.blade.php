<div x-data="{ showDropdown: @entangle('showDropdown'), showCreateModal: @entangle('showCreateModal') }"
    ...
    <input
        ...
        class="block px-2.5 pb-2.5 pt-4 w-full text-xs text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:bg-dark dark:text-dark-9
                    ...
                    <ul class="overflow-y-scroll h-44 text-xs">
                        @forelse ($results as $index => $transport)
                            <li wire:click="selectTransport(@js($transport))"
                                class="cursor-pointer px-3 py-1 hover:font-bold dark:bg-dark dark:text-dark-9 dark:hover:bg-dark-3 hover:bg-zinc-100 text-blue-900 h-fit ml-2 mr-2 rounded-md
